...
SoItem.initColumn = function () {
    return [
        {field: 'selectItem', radio: true},
            ...
            {title: '平台规格品id', field: 'platformSkuId', visible: true, align: 'center', valign: 'middle'},
            ...
            {title: '商品总金额', field: 'orderItemAmount', visible: true, align: 'center', valign: 'middle'},
            ...
            {title: '产品中文名称', field: 'productCname', visible: true, align: 'center', valign: 'middle'},
 ...
 * 检查是否选中
 */
SoItem.check = function () {
    var selected = $('#' + this.id).bootstrapTable('getSelections');
    if(selected.length == 0){
        Feng.info("请先选中表格中的某一记录！");
        return false;
    }else{
        SoItem.seItem = selected[0];
        return true;
    }
};
...
SoItem.openAddSoItem = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/soItem/soItem_add'
    });
 ...
 * 打开查看详情
 */
SoItem.openSoItemDetail = function () {
    if (this.check()) {
        var index = layer.open({
            type: 2,
            title: '详情',
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/soItem/soItem_update/' + SoItem.seItem.id
        });
        this.layerIndex = index;
    }
};
 ...
 * 删除
 */
SoItem.delete = function () {
    if (this.check()) {
        var ajax = new $ax(Feng.ctxPath + "/soItem/delete", function (data) {
            Feng.success("删除成功!");
            SoItem.table.refresh();
        }, function (data) {
            Feng.error("删除失败!" + data.responseJSON.message + "!");
        });
        ajax.set("soItemId",this.seItem.id);
        ajax.start();
    }
};
 ...
 * 查询列表
 */
SoItem.search = function () {
    var queryData = {};
    queryData['condition'] = $("#condition").val();
    SoItem.table.refresh({query: queryData});
};
