<template>
    <div>
        ...
        > 
            ...
            <order-detail v-if="dialogType === 2" :originalCode="originalCode"
                          :orderCode="orderCode" :action="action"></order-detail>
...
<script>
    ...
    import ModifyReceivingAddress from './ModifyReceivingAddress'
    ...
    export default {
        name: 'userManage',
        components: {
            VPageTable, ViewLog, OrderDetail, ModifyReceivingAddress, OrderRemark
        },
        data() {
            return {
                searchData: {},
        ...
        methods: {
            ...
            handleSearchTopBtn(methodName) {
                ...
                this.$Api[this.$Methods.SO_ORDER_EXPORT_TRUE_OUT](searchData).then((resp) => {
                    if (resp.status === "1") {
                ...
                //     this.$Config.MessageModal("warning", "请选中其中一条数据！")
                // } else {
                    this.$Config.ConfirmModal("确定审核吗？", () => {
                        this.$Api[this.$Methods.SO_ORDER_PASS_STATUS]({soOrderIds: idList}).then((resp) => {
                            if (resp.status === "1") {
