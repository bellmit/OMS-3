...
public class DownloadAndZipFileUtil {
    public static Logger logger = LoggerFactory.getLogger(DownloadAndZipFileUtil.class);
...
//                  //最大的流为10M
    ...
    public static void downloadFile( String fileName,File file, HttpServletResponse response, boolean isDelete) {
        try {
            // 以流的形式下载文件。
            String fileResultName = new String(fileName.getBytes("UTF-8"), "ISO-8859-1");
            ...
            response.setContentType("application/octet-stream");
            response.setHeader("Content-Disposition", "attachment;filename=" + fileResultName);
